    ...
    if request.method == 'POST':
        ...
        if form.validate() == True:
            ...
            with youtube_dl.YoutubeDL() as ytdl:
                ...
                ytdl_format_options = {'format': 'bestaudio/best', 'outtmpl': '/home/mforbes5223/web_app/web_app/media' + "/"  + sc_artist + " - " + sc_title + "." + sc_ext,'quiet':True}
            with youtube_dl.YoutubeDL(ytdl_format_options) as ytdl:
                ...
                sc_ext = info['ext']
                sc_ext = sc_ext.replace(":","").replace("<","").replace(">","").replace('"',"").replace("/","").replace("\\","").replace("|","").replace("?","").replace("*","")
                return send_file(filename_or_fp= '/home/mforbes5223/web_app/web_app/media' + "/" + sc_artist + " - " + sc_title + "." + sc_ext,mimetype='audio/mpeg',as_attachment= True, attachment_filename=sc_artist + " - " + sc_title + "." + sc_ext)
        else:
